...
let read_exp eq = 
  ...
  | Eram (_, _, a1, a2, a3, a4)	-> 
     read_arg a1 @ read_arg a2 @ read_arg a3 @ read_arg a4
  | Econcat (a1, a2)		-> read_arg a1 @ read_arg a2
  | Eslice (_, _, a)		-> read_arg a
  | Eselect (_, a)              -> read_arg a

let schedule p =   
  let find_equation ident = 
    try
      [List.find (fun eq -> (fst eq) = ident) p.p_eqs]
    with Not_found -> []
  in
  ...
  let sorted_equations = 
    List.fold_left
      ( fun eq_ls id -> (find_equation id) @ eq_ls) [] sorted_ident in
